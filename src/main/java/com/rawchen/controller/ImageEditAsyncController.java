...
public class ImageEditAsyncController {
    ...
    /**
     * 异步局部改图 - 前端已通过OSS上传原图和遮罩图
     *
     * @param originalImageUrl 原图的OSS URL
     * @param maskImageUrl      遮罩图的OSS URL（纯橘色选区+透明背景）
     * @param prompt            编辑提示词
     * @param model             使用的模型（默认 gpt-image-2）
     * @param user              当前登录用户
     * @return 任务ID
     */
    @PostMapping("/edit_async")
    public R<EditAsyncResponse> editImageAsync(
        ...
        if (maskImageUrl == null || maskImageUrl.trim().isEmpty()) {
            return R.badRequest("请绘制选区");
        }
        ...
        consumeLogService.createLog(user.getId(), taskId, "edit", model,
                price != null ? price.getModelName() : model, null, cost);

        // 异步执行任务
        asyncImageTaskExecutor.executeEditInpaintTask(taskId, originalImageUrl, maskImageUrl, prompt, model);

        log.info("Edit inpaint task {} created for user {} with model {}, cost {}", taskId, user.getId(), model, cost);

        EditAsyncResponse response = new EditAsyncResponse();
    ...
    public R<TaskHistoryPageResponse> getHistory(
        ...
        IPage<ImageTask> taskPage = imageTaskService.getUserTaskPage(user.getId(), page, size, "edit", status);
     ...
     */
    @Data
    public static class EditAsyncResponse {
