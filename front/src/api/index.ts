...
export const imageRestoreApi = {
  // 异步老照片修复
  restoreImageAsync: (originalImageUrl: string, colorMode: string, grainMode: string, clarity: string, model: string = 'gpt-image-2') => {
    const params = new URLSearchParams();
    params.append('originalImageUrl', originalImageUrl);
    params.append('colorMode', colorMode);
    params.append('grainMode', grainMode);
    params.append('clarity', clarity);
    params.append('model', model);
    return api.post<{ taskId: string; model: string }>('/image-restore/restore_async', params, {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    }) as unknown as Promise<{ taskId: string; model: string }>;
  },
  ...
  getTaskResult: (taskId: string) =>
    api.get<{
      ...
      originalImageUrl?: string;
      msg?: string;
    }>('/image-restore/result', { params: { id: taskId } }) as unknown as Promise<{
      status: 'pending' | 'done' | 'error' | 'not_found';
      imageUrl?: string;
      originalImageUrl?: string;
      msg?: string;
  ...
  getHistory: (page = 1, size = 12, status?: string) =>
    api.get<{
      ...
      current: number;
      size: number;
    }>('/image-restore/history', { params: { page, size, status } }) as unknown as Promise<{
      records: ImageTaskRecord[];
      total: number;
      pages: number;
      current: number;
...
export const imageEditApi = {
  // 异步局部改图
  editImageAsync: (originalImageUrl: string, maskImageUrl: string, prompt: string, model: string = 'gpt-image-2') => {
    const params = new URLSearchParams();
    params.append('originalImageUrl', originalImageUrl);
    params.append('maskImageUrl', maskImageUrl);
    params.append('prompt', prompt);
    params.append('model', model);
    return api.post<{ taskId: string; model: string }>('/image-edit/edit_async', params, {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    }) as unknown as Promise<{ taskId: string; model: string }>;
  },
  ...
  getTaskResult: (taskId: string) =>
    api.get<{
      ...
      maskImageUrl?: string;
      msg?: string;
    }>('/image-edit/result', { params: { id: taskId } }) as unknown as Promise<{
      status: 'pending' | 'done' | 'error' | 'not_found';
      imageUrl?: string;
      originalImageUrl?: string;
      maskImageUrl?: string;
  ...
  getHistory: (page = 1, size = 12, status?: string) =>
    api.get<{
      ...
      current: number;
      size: number;
    }>('/image-edit/history', { params: { page, size, status } }) as unknown as Promise<{
      records: ImageTaskRecord[];
      total: number;
      pages: number;
      current: number;
  ...
  // 获取任务详情
  getTaskDetail: (taskId: string) =>
    api.get<ImageTaskRecord>(`/image-edit/task/${taskId}`) as unknown as Promise<ImageTaskRecord>,
};
