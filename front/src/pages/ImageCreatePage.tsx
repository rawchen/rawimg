...
export function ImageCreatePage() {
  ...
  const [generateCount, setGenerateCount] = useState(1); // 生成数量，默认为1
  ...
  const [currentInspiration, setCurrentInspiration] =
    useState<InspirationTemplate | null>(null);
  ...
  const [selectedHistory, setSelectedHistory] =
    useState<ImageTaskRecord | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // 根据当前模型和分辨率计算实际使用的模型
        const effectiveModel = getEffectiveModelCode(
          selectedModel,
          selectedResolution,
        );
  ...
  const handleSubmit = async () => {
      ...
      if (textarea) {
        ...
        requestAnimationFrame(() => {
          ...
          const targetScroll = Math.max(
            0,
            ratio * textarea.scrollHeight - textarea.clientHeight * 0.3,
          );
    ...
    try {
      // 构建参考图URL JSON数组
      const referenceUrls =
        uploadedOssUrls.length > 0
          ? JSON.stringify(uploadedOssUrls)
          : undefined;

      // 使用异步接口（只传递URL，不传递文件）
      const actualSize = getActualSize(selectedSize, selectedResolution);
      const effectiveModel =
        selectedModel === "gpt-image-2" ? undefined : selectedModel;
      const { taskId } = await imageCreateApi.createImageAsyncWithUrls(
        referenceUrls,
        prompt,
        actualSize,
        effectiveModel,
      ...
      <div className="flex-1 bg-[#F5F7FA]">
        <div className="max-w-7xl mx-auto px-4 py-6 md:py-10">
          ...
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            {/* 左侧结果展示区域 */}
            <div className="lg:w-[56%] order-2 lg:order-1" ref={fancyboxRef}>
              {/* 生成的图片展示 */}
              {(() => {
                // 解析createdImage为图片数组
                const imageUrls = createdImage
                  ? createdImage.split(",").filter((url) => url.trim())
                  : [];
                  ...
                  <div className="bg-white rounded-xl px-6 py-4 flex flex-col items-center gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-700">
                        任务已提交，可稍后在生成历史查看{" "}
                        {formatDurationFromSeconds(currentElapsed)}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/*<div className="bg-white rounded-xl p-4 shadow-sm">*/}
                <div className="flex flex-wrap items-center gap-2 pt-4">
                  ...
                  {shouldShowResolutionToggle() && (
                    <div
                      className={`relative flex items-center rounded-lg p-1 ml-1 ${
              ...
              <div className="mt-4">
                ...
                <div className="relative">
                  ...
                  >
                    {/* 复制两份实现无缝滚动 */}
                    {[...randomTemplates, ...randomTemplates].map(
                      (template, index) => (
                        <div
      ...
      >
        <div className="relative">
          ...
          {(!templates || templates.length === 0) && !loadingTemplates ? (
            <div className="text-center py-8 text-gray-400">暂无模板数据</div>
          ) : (
            <div>
                        ...
                        <div className="absolute bottom-2 right-2 w-12 h-12 rounded border border-white shadow-sm overflow-hidden">
                          <img
                            src={
                              addOssThumbnailStyle(
                                ensureHttpsUrl(
                                  JSON.parse(record.referenceImageUrls)[0],
                                ),
                              ) || ""
                            }
                            alt="参考图"
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {record.status === "pending" ? (
                            <span className="text-gray-500">
                              {getPendingElapsed(record.createTime)}
                            </span>
                          ) : (
        ...
        {selectedHistory && (
          <div>
            ...
            <div className="flex items-center gap-4 mb-4 text-xs text-gray-500">
              ...
              <span>
                类型:{" "}
                {selectedHistory.taskType === "create" ? "创作" : "增强"}
              </span>
